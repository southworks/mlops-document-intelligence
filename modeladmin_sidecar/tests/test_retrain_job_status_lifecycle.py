...
def test_get_running_job_syncs_succeeded_from_adi(tmp_path) -> None:
    ...
    with patch(
        "modeladmin_sidecar.routes.retrain_jobs.DocumentIntelligenceService.get_compose_status",
...
def test_get_terminal_job_does_not_call_adi(tmp_path) -> None:
    ...
    with patch(
        "modeladmin_sidecar.routes.retrain_jobs.DocumentIntelligenceService"
    ) as mock_adi_service:
        # GET the job
        response = client.get(f"/modeladmin/retrain-jobs/{job_id}")
        assert response.status_code == 200
